...
class UDPClient {
	...
	public static int receiveTimer = 250;
	...
	public static boolean isACK(int seqNo, byte[] data) {
		try {
			...
			ackTmp = dataBuf.get();
			if ((seqTmp == seqNo) && (notcorrupt(data)) && (ackTmp == ACK))
				return true;
			return false;
		} catch (Exception e) {
			System.out.println("Error:" + e);
	...
	public static void rdt_send(ByteBuffer data) {
		...
		try {
			...
			while (true) {
				data.get(packetData);
				byte[] packet = make_pkt(sequenceNo, packetData, CheckSum(sequenceNo, packetData));
				udt_send(packet);
				while (!isACK(sequenceNo, rdt_rcv(ackPacketSize,packet))) {
			...
			byte[] packetLast = make_pkt(sequenceNo, packetDataLast, CheckSum(sequenceNo, packetDataLast));
			udt_send(packetLast);
			while (!isACK(sequenceNo, rdt_rcv(ackPacketSize,packetLast))){
				packetLast = make_pkt(sequenceNo, packetDataLast, CheckSum(sequenceNo, packetDataLast));
				udt_send(packetLast);
			}
			sequenceNo++;
			clientSocket.close();
	...
	public static byte[] rdt_rcv(int pktSize,byte[] udpPacket) {
		try {
			...
			clientSocket.receive(receivePacket);
				...
				udt_send(udpPacket);
				packet = rdt_rcv(pktSize,udpPacket);
